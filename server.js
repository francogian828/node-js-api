/*
Node.js is built on an event-driven, non-blocking I/O model, which allows it to handle a large number of concurrent requests efficiently. 
The non-blocking nature means that I/O operations (such as reading files, querying a database, etc.) don’t block the execution of other code.
...
app.get('/users/:id', async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).send({ error: 'User not foundd' });
        res.send(user);
    } catch (error) {
